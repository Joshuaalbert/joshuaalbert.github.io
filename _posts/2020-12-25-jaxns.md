...
cover-img: /assets/img/jaxns_logo.png
thumbnail-img: /assets/img/jaxns_logo.png
share-img: /assets/img/jaxns_logo.png
...
This project stems from a [problem](/2020-03-01-systematics) that I encountered during my research on ionospheric calibration during my Ph.D..
...
In simulated annealing MC the likelihood is modified with a temperature-like variable, ![eqn](https://latex.codecogs.com/svg.latex?p(y\mid\theta)&space;\to&space;\left(p(y\mid\theta)\right)^\beta), where ![eqn](https://latex.codecogs.com/svg.latex?\beta) is inverse-temperature and ranges from zero to one.
When ![eqn](https://latex.codecogs.com/svg.latex?\beta=0) the HMC just samples the prior, and when ![eqn](https://latex.codecogs.com/svg.latex?\beta=1) the HMC samples the true posterior.
Thus, simulated annealing sequentially raises ![eqn](https://latex.codecogs.com/svg.latex?\beta) from zero to one in steps, using the results of the previous step to initialise the chains in the current step.
